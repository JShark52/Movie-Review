...
def home(request):
    ...
    banner = Movie.objects.all().filter(averageRating=8)[:3]
...
def detail(request, id):
    movie = Movie.objects.get(id=id) # selct * from movie where id=id
...
def edit_review(request, movie_id, review_id):
    if request.user.is_authenticated:
        ...
        #check if the review was done by logged in user
